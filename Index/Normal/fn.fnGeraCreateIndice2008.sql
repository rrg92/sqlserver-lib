/*#info 
	...
	# Detalhes 
		Outra versão de função que gera o create dos  índices, funcionando no 2008.
...
IF object_id('dbo.fnGeraCreateIndice2008') IS NOT NULL
	DROP FUNCTION dbo.fnGeraCreateIndice2008
GO

CREATE FUNCTION dbo.fnGeraCreateIndice2008
(
		...
		CONVERT(varchar(max),
			(SELECT --> Monta a lista de colunas, tanto key-columns, como INCLUDE. (Na ordem correta!)
				...
				CASE --> Este case irá controlar quando deve ser colocado o parênteses de fechamento do INCLUDE ou do inicial!!!!
					--> Deve se verificar o final do INCLUDE primeiro, pois essas colunas sempre tem key-ordinal = 0
					/**RRG27032012-01**/
					WHEN ROW_NUMBER() OVER(PARTITION BY IC.is_included_column ORDER BY IC.column_id DESC) = 1 
							AND IC.is_included_column = 1 THEN ') '
					WHEN ROW_NUMBER() OVER(ORDER BY IC.is_included_column,IC.key_ordinal DESC)	= 1 THEN ') '
					ELSE ''
				END  AS 'text()'
			FROM
